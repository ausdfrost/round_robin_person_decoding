...
group_dir=/projects/csnl/shared
container=containers/fmriprep-latest-2018-09-05.simg #poldracklab_fmriprep_latest-2017-12-07-ba92e815fc4e.img
freesurferlicense=/projects/csnl/shared/containers/license.txt
study=round_robin_bids

# Set subject list
sublist=`cat subject_list.txt`
...
sbatch --export ALL,subid=${sub},group_dir=${group_dir},study=${study},container=${container},freesurferlicense=${freesurferlicense} \
	...
	--mem=100G \
	--account=csnl \
	-o ${group_dir}/${study}/fmriprep/output/${SUBID}_${SESSID}_fmriprep_output.txt \
	-e ${group_dir}/${study}/fmriprep/output/${SUBID}_${SESSID}_fmriprep_error.txt \
	job_fmriprep.sh
done
