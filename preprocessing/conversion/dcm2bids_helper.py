...
def main():
    ...
    if os.path.isdir(cfg.path_dicoms):
        ...
        if cfg.run_local:
            cmd = 'dcm2bids_helper --force -d {path_dicoms}/{test_subject} -o {path_bidsdata}'.format(path_dicoms=cfg.path_dicoms, test_subject=cfg.test_subject, path_bidsdata=cfg.path_bidsdata) # THIS IS CALLED # changed the -b flag to -o
        else:
        ...
        print(cmd)
        subprocess.call([cmd], shell=True) # TODO this is where the 'command not found' error is
    else:
...
def create_logfiles(logfile_fullpaths:list):
    ...
    @param logfile_fullpaths:        Paths to log files to check
    """
